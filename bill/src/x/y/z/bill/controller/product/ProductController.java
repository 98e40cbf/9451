...
public class ProductController extends BaseController {
    ...
    public String listing(final ModelMap model) {
        ...
        PageResultDTO<ProductDTO> resultDTO = productService.selectProductList(productDTO);
        model.put("productList", resultDTO);

        model.addAttribute("pageNum", productDTO.getPageNum() + 1);
        int totalPage = resultDTO.getTotalRow() % productDTO.getPageSize() == 0
                ? resultDTO.getTotalRow() / productDTO.getPageSize()
    ...
    @ResponseBody
    public PageResultDTO<ProductDTO> productList(@PathVariable final int pageNum, @PathVariable final int pageSize) {
        ProductDTO productDTO = new ProductDTO();
